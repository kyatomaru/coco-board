...
export default function CourtView({ board, onClose, onSubmit, frame, setFrame, currentFrame, setCurrentFrame, isPlay, isView, setIsPlay, playFrame }: PageProps) {
    ...
    return (
        <Box id="mainCourt" onTouchStart={(e) => { document.addEventListener("touchmove", scrollNo, { passive: false }); }}
            onTouchEnd={(e) => { document.removeEventListener("touchmove", scrollNo); }} >
            {!isView &&
                <>
