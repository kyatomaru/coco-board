...
export default function ReviewBox({ user, allContents, content, setContents, boards }: PageProps) {
    ...
    const DeleteGameContent = async () => {
        ...
        const res = await useDeleteGame(content.contentsId)
        if (res.ok) {
            ...
            {!isEditLoading ? (
                <>
                    {editModalOpen ? (
                        ...
                        <Box sx={{ backgroundColor: "#fbfbfb", borderRight: "solid 0.5px #b2b2b2", borderLeft: "solid 0.5px #b2b2b2" }}>
                            <Box sx={{
