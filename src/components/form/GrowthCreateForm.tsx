...
export default function GrowthFormModal({ open, setOpen, getGrowth }: PageProps) {
    ...
    const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const res = await insertGrowthData(growthContents)
        if (res.ok) {
            getGrowth()
            setOpen(false)
        } else {
...
async function insertGrowthData(contents) {
