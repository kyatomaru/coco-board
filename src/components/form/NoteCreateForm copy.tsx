...
import GameInfoForm from './game/GameForm';
            ...
            >
                    ...
                    {menu == 0 ?
                        <>{gameMenu == 0 ?
                            <Grid sx={{ px: 1, height: "50px" }} container direction="row" alignItems="center" justifyContent="space-between">
                                ...
                                <Grid >
                                    <Button size="small" sx={{ color: 'black' }} variant='text' onClick={(event) => { setTimeout(() => { setGameMenu(1) }, 400) }}>次へ</Button>
                                </Grid>
                            </Grid>
                            : (gameMenu == 1) &&
                            <Grid sx={{ px: 1, height: "50px" }} container direction="row" alignItems="center" justifyContent="space-between">
                                <Grid >
                                    <Button size="small" sx={{ color: 'black' }} variant='text' onClick={(event) => { setTimeout(() => { setGameMenu(0) }, 400) }}>戻る</Button>
                                </Grid>
                                <Grid >
                ...
                <Box sx={{ pt: 1 }}>
                            ...
                            <Box sx={{ mx: "auto", px: 2 }}>
                                <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ja}>
                                    <DemoContainer components={['DatePicker']}>
                                        <DatePicker sx={{ width: "auto !important", backgroundColor: "background.paper" }} format='yyyy年MM月dd日' value={dateValue} disableFuture onChange={(newValue) => setDateValue(newValue)} />
                                    </DemoContainer>
                    ...
                    {menu == 0 &&
                        <>
                            {gameMenu == 0 ?
                                <GameInfoForm contents={gameContents} titleError={titleError} />
                                : (gameMenu == 1) &&
                                <GameProblemForm contents={gameContents} titleError={titleError} />
                            }
                        </>
                    }
                    {menu == 1 &&
