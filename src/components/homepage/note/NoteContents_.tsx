...
const DataFormat = (date) => {
    ...
    const thisYesterday = dayjs(String(new Date().setDate(date.getDate() - 1))).format('YYYY/M/DD')

    if (thisToday == recodeDate) return "今日"
    if (thisToday == recodeDate) return "昨日"
...
export default function NoteContents({ contents, DeleteContents }: PageProps) {
    ...
    return (
        <Card sx={{ minWidth: 250, my: 1 }} elevation={2}>
